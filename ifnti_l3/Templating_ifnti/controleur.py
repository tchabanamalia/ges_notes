...
def generate3_pdf(context) :

    '''INSTANCIATION DUN NOUVEL ENVIRONNEMENT AVEC DES OPTIONS DE BALISES PERSONNALISÉES'''
    ...
    fichier_in = open("ifnti/noteEleves_par_matiere.tex", 'r')
    ...
    mon_pdf.save_to("out/noteEleves_par_matiere.pdf")
...
def generate2_pdf(context) :
    ...
    '''DECLARATION DE FICHIER'''
    #fichier à lire contenant le template avec les balises
    fichier_in = open("ifnti/liste_eleves_par_niveau.tex", 'r')
    ...
    fichier_out.close()
    mon_pdf = build_pdf(open("out/template_out.tex", 'r'))
    mon_pdf.save_to("out/liste_eleves_par_niveau.pdf")
